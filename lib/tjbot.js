...
TJBot.prototype._setupLED = function() {
    ...
    var ws = new (require('i2c-ws2812-adapter'))();

    // init with 1 LED
    this._led = ws;
    this._led.setPixelCount(1)
    .then(() => { winston.debug("Set up pixel count")})
    .catch(e => winston.warn("Failed to set pixel count:", e));
...
TJBot.prototype.shine = function(color) {
    ...
    var r = parseInt("0x"+rgb.substring(1,3));
    var g = parseInt("0x"+rgb.substring(3,5));
    var b = parseInt("0x"+rgb.substring(5,7));

    // shine!
    winston.verbose("TJBot shining my LED to RGB color " + r + "," + g + "," + b);

    // set the LED color
    this._led.setPixelColor(0,r,g,b)
    .then(() => { this._led.send(); })
    .catch((reason) => { winston.warn("Failed to set LED color:", reason)});
}
